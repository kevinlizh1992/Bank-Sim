...
public class Server extends Thread{
     ...
     */
     ...
     {   int accIndex;             	/* Index position of account to update */
         ...
         { //because we only yield if theres actually more stuff to be done by sender
        	  while((objNetwork.getInBufferStatus().equals("empty")) && !objNetwork.getClientConnectionStatus().equals("disconnected")) {Thread.yield();}   /* Alternatively, busy-wait until the network input buffer is available */
        	 ...
        	 if (!objNetwork.getInBufferStatus().equals("empty"))
        	 {
        		...
        		//because we only yield if theres actually more stuff to be done by sender
        		  while( (objNetwork.getOutBufferStatus().equals("full")) && !objNetwork.getServerConnectionStatus().equals("disconnected")) {Thread.yield();} /* Alternatively,  busy-wait until the network output buffer is available */
    ...
    {   
    	...
    	long serverStartTime, serverEndTime = 0;
       ...
       // System.out.println("server "+ objNetwork.getServerConnectionStatus());
